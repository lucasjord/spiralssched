...
c             for AuScope+Ceduna...
	      if ( n_scan.eq.1 .and.
     +            (stat_code.eq.'HB' .or. stat_code.eq.'KE' .or.
     +             stat_code.eq.'YG' .or. stat_code.eq.'CD') ) then
		 az_check = az
c                force between -180 and +180 deg
		 if ( az.lt.-180.d0 ) az_check = az+360.d0
		 if ( az.gt.+180.d0 ) az_check = az-360.d0
		 if ( az_check.lt.-49.d0 .or.
     +                az_check.gt.179.d0      ) then
		    bad_starting_az = .true.
		 endif
	      endif
