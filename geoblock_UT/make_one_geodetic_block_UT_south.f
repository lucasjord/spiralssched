...
C	Version tested with gfortran compiler; Mark J. Reid  July, 2016
	...
	n_ref_ant = 1               ! reference antenna #, may want to play with this
...
c       Add header lines to output files
	call input_stations ( stat_codes )
...
c==============================================================
	subroutine input_stations ( stat_codes )
...
C	Generate a trial geodetic-like data set...
	...
	max_dropped = 5       ! maximum number sources not observed at a station
	angle_min   = 12.d0   ! [deg] minimum angular separation among sources
...
c	ut_days = UT_hrs / 24.d0           ! [days]
	cd_lst_hrs = cd_lst_start_hrs      ! starting LST time at CD [hrs]
...
c       If first scan, set "old" azimuth, elevation values to 180,45 degrees.
	if ( n_scan .eq. 1 ) then
	   do i_a = 1, n_stats
...
c             for Ceduna+Wark
	      if ( n_scan.eq.1 .and.
     +            (stat_code.eq.'CD') .or.
     +             stat_code.eq.'WA' ) then
		 az_check = az
c                force between -180 and +180 deg
c		 if ( az.lt.-180.d0 ) az_check = az+360.d0
c		 if ( az.gt.+180.d0 ) az_check = az-360.d0
c		 if ( az_check.lt.-180.d0 .or.
c     +                az_check.gt.300.d0      ) then
...
c           Special case for Parkes 30deg elevation limit...
...
c   Replace az_new with possible new wrap value...
	...
	if ( stat_code .eq. 'WA' ) az_rate = 21.d0 ! added by ljh
...
c       Calc slew time and add "start,acceleration,settle" time
	...
	if ( stat_code .eq. 'WA' ) el_rate = 21.d0
