...
c       This version adjusts the quality factor to favor having the
c       last source have an azimuth near the specified initial values
c       for CD and WA.      
...
c          Adjust "quality" factor to disfavor having the last azimuth
c          far from the initial azimuth (given in the stations file)
c          for WA (slowest slewing of SPiRALs antennas)
           call degrade_quality( n_stats, stat_codes, n_scans, quality,
     +          az_saved, sazimuth_initial, degrade_factor )
           quality = quality * degrade_factor
...
cV7============================================================
      subroutine degrade_quality ( n_stats, stat_codes,
     +             n_scans, quality,
     +             az_saved, sazimuth_initial, degrade_factor )

        implicit real*8 (a-h,o-z)
        
	real*8       az_saved(40,20)
        real*8       sazimuth_initial(40) 
	character*2  stat_codes(40)

c       Get WA station number
        n_WA = 0
        do n = 1, n_stats
           if ( stat_codes(n) .eq. 'WA' ) n_WA = n
        enddo

        WA_desired_az  = sazimuth_initial(n_WA)
        WA_lastscan_az = az_saved(n_scans,n_WA)
        del_az = abs(WA_desired_az - WA_lastscan_az)

        degrade_factor = exp(del_az/60.d0)            
